...
const IconCap = (p: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" width="24" height="24" {...p} fill="none" stroke="currentColor" strokeWidth="2"
...
export default function Services({ t }: { t: ServicesCopy }) {
  ...
  return (
    <Section id="services">
      {/* Header */}
      <div className={`flex items-end justify-between ${isRTL ? "text-right" : ""}`}>
        <div>
...
/>
            ...
            <div className={`flex items-center ${isRTL ? "justify-start" : "justify-end"}`}>
              <button
                className="text-sm inline-flex items-center gap-1 px-3 py-1.5 rounded-xl border bg-white hover:bg-slate-50 transition"
                aria-label={isRTL ? `المزيد عن ${s.title}` : `Learn more about ${s.title}`}
                type="button"
              >
                {isRTL ? "المزيد" : "Learn more"}
                <span className={`transition-transform group-hover:translate-x-0.5 ${isRTL ? "-scale-x-100" : ""}`}>
                  →
                </span>
              </button>
            </div>
